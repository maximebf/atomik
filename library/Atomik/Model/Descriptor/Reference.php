 ...
 * @subpackage Model
 */
class Atomik_Model_Descriptor_Reference
{
	const HAS_ONE = 'one';
	 ...
	 * 
	 * @param	string|Atomik_Model_Descriptor|Atomik_Model 	$target
	 * @return 	bool
	 */
	public function isTarget($target)
	{
		...
		if (!($target instanceof Atomik_Model_Descriptor)) {
			$target = Atomik_Model_Descriptor_Factory::get($target);
		}
	...
	/**
	 * Returns the descriptor of the target
	 * 
	 * @return Atomik_Model_Descriptor
	 */
	public function getTargetDescriptor()
	{
		return Atomik_Model_Descriptor_Factory::get($this->target);
	}
	...
	public function getQuery(Atomik_Model $sourceModel)
	{
		...
		$query->from($this->getTargetDescriptor())->where(array($this->targetField => $sourceModel->{$this->sourceField}));
		return $query;
	}
}