...
class Atomik_Db_Instance
{
	...
	/**
	 * @param Atomik_Db_Query_Generator_Interface $generator
	 */
	public function setQueryGenerator(Atomik_Db_Query_Generator_Interface $generator)
	{
	    $this->_queryGenerator = $generator;
	}
	...
	{
	    if ($this->_queryGenerator === null) {
	        $this->_queryGenerator = new Atomik_Db_Query_Generator();
	    }
	 ...
	 * 
	 * If $where is null, $data will also be used for the where clause
	 * If $where is a string, the value from $where will become array($where => $data[$where])
	 * If $where is an array, it will be used as the where clause. If some indexes are numbers and
	 * their value is a string, the same thing as when $where is a string will happen.
	 * 
	 * @param string $table
	 * @param array $data
