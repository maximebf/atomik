<?php
	...
	config_merge(array(
		...
		'core_paths_root'				=> dirname(__FILE__) . '/',
		'core_paths_plugins'			=> dirname(__FILE__) . '/plugins/',
		'core_paths_actions' 			=> dirname(__FILE__) . '/actions/',
		'core_paths_templates'	 		=> dirname(__FILE__) . '/templates/',
		'core_paths_includes'			=> dirname(__FILE__) . '/includes/',
	...
	if (!isset($_GET[config_get('core_url_trigger')]) || empty($_GET[config_get('core_url_trigger')])) {
		...
		/* checking if no dot are in the page name to avoid any hack attempt */
		if (strpos($url, '..') !== false) {
			trigger404();
		}
	...
	if (file_exists(config_get('request_template'))) {
		events_fire('core_before_template');
		
		/* render template */
		ob_start();
		include(config_get('request_template'));
		$output = ob_get_clean();
	 ...
	 */
	function core_end($success = false)
	{
